...
void rospc_to_o3d(const sensor_msgs::PointCloud2ConstPtr& ros_pc2, open3d::geometry::PointCloud& o3d_pc)
{
  ...
  {
    if (ros_pc2->fields[3].name == "intensity")
    {
      sensor_msgs::PointCloud2ConstIterator<float> ros_pc2_intensity(*ros_pc2, "intensity");
      for (size_t i = 0; i < ros_pc2->height * ros_pc2->width;
           ++i, ++ros_pc2_intensity, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z)
      {
        o3d_pc.points_.push_back(Eigen::Vector3d(*ros_pc2_x, *ros_pc2_y, *ros_pc2_z));
      }
    }
    else if (ros_pc2->fields[3].name == "rgb")
    {
      ...
      {
        o3d_pc.points_.push_back(Eigen::Vector3d(*ros_pc2_x, *ros_pc2_y, *ros_pc2_z));
        o3d_pc.colors_.push_back(Eigen::Vector3d(*ros_pc2_r / 255, *ros_pc2_g / 255, *ros_pc2_b / 255));
      }
    }
  }
                  ...
                  std::string frame_id)
{
  ...
  if (pointcloud.HasColors())
  {
         ...
         i++, ++ros_pc2_x, ++ros_pc2_y, ++ros_pc2_z, ++ros_pc2_r, ++ros_pc2_g, ++ros_pc2_b)
    {
      ...
      *ros_pc2_y = point(1);
      *ros_pc2_z = point(2);
      *ros_pc2_r = ((int)(255 * color(0)) & 0xff) >> 16;
      *ros_pc2_g = ((int)(255 * color(1)) & 0xff) >> 8;
      *ros_pc2_b = ((int)(255 * color(2)) & 0xff);
    }
  }
  else
  {
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subCloud = nh.subscribe("/transformed_cloud", 1, cloud_callback);
  ros::spin();
  // sensor_msgs::PointCloud2 rospc2;
  // Example XYZ Pointcloud
  ...
  /*       open3d::geometry::PointCloud pointcloud;
          pointcloud.points_.push_back(Eigen::Vector3d(0.0, 0.0, 0.0));
          pointcloud.colors_.push_back(Eigen::Vector3d(0, 1, 0));
          pointcloud.points_.push_back(Eigen::Vector3d(1.0, 0.0, 0.0));
          pointcloud.colors_.push_back(Eigen::Vector3d(0, 0, 1));
          pointcloud.points_.push_back(Eigen::Vector3d(0.0, 1.0, 0.0));
          pointcloud.colors_.push_back(Eigen::Vector3d(0, 1, 0));
          pointcloud.points_.push_back(Eigen::Vector3d(0.0, 0.0, 1.0));
          pointcloud.colors_.push_back(Eigen::Vector3d(1, 0, 0));
